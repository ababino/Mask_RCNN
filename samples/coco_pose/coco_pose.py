...
class CocoPoseDataset(utils.Dataset):
    def load_coco(self, dataset_dir, subset, year=DEFAULT_DATASET_YEAR, class_ids=None,
        ...
        # Add classes
        for i in class_ids:
            cat_data = coco.loadCats(i)[0]
            self.add_class("coco", i, cat_data["name"])

        # Add skeleton and keypoints to class_info
        self.max_keypoints = 0
        for class_info in self.class_info:
            class_id = class_info['id']
            if class_id in class_ids:
                coco_class_info = coco.loadCats(class_id)[0]
    ...
    def load_mask_and_keypoint(self, image_id):
        ...
        for annotation in annotations:
            ...
            if class_id:
                ...
                # create keypoint mask for the instance.
                class_info = self.class_info[class_id]
                mask = np.zeros((image_info["height"], image_info["width"], self.max_keypoints))
                if 'keypoints' in class_info:
                    kp = np.array(annotation['keypoints'])
                    x = kp[0::3]
                    y = kp[1::3]
                    v = kp[2::3]
                ...
                if mask.max() < 1:
                    continue
                instance_masks_keypoints.append(mask)
        ...
        if class_ids:
            ...
            keypoint_mask = np.stack(instance_masks_keypoints, axis=3).astype(np.bool)
            class_ids = np.array(class_ids, dtype=np.int32)
        else:
            mask = np.empty([0, 0, 0])
            keypoint_mask = np.empty([0, 0, 0, 0])
            class_ids = np.empty([0], np.int32)
        return keypoint_mask, mask, class_ids
        ...
        for annotation in annotations:
            ...
            if class_id:
                if 'keypoints' in class_info:
                    ...
                    instance_keypoints.append(bones)
                    class_ids.append(class_id)

        # Pack instance masks into an array
        if class_ids:
            class_ids = np.array(class_ids, dtype=np.int32)
            return instance_keypoints, class_ids
        else:
