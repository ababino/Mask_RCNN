...
def unmold_keypoint_mask(kp_mask, bbox, image_shape):
    """Converts a keypoint mask generated by the neural network to a format similar
    to its original shape.
    mask: [height, width, num_keypoints] of type float. A small, typically 56x56 mask.
    bbox: [y1, x1, y2, x2]. The box to fit the mask in.
    ...
    threshold = 0.1
    y1, x1, y2, x2 = bbox
    height, width, num_keypoints = kp_mask.shape
    kp_mask_exp = np.exp(kp_mask)
    partition = kp_mask_exp.sum(axis=(0, 1), keepdims=True)
    kp_mask_softmax = kp_mask_exp / partition
    below_th_mask = kp_mask_softmax < threshold

    kp_mask_softmax_max = kp_mask_softmax.max(axis=(0, 1), keepdims=True)

    kp_mask_bin = (kp_mask_softmax == kp_mask_softmax_max).astype(float)
    kp_order = np.linspace(0., 1., height * width).reshape((height, width, 1))
    kp_mask_bin += kp_order
    threshold = kp_mask_bin.max(axis=(0, 1), keepdims=True)
    kp_mask_bin = kp_mask_bin == threshold
    kp_mask_bin[below_th_mask] = 0
